...
function buildEmailHtml(payload: NotifyPayload): string {
  ...
  const snippet = payload.textSnippet
    ? `<blockquote style="margin:16px 0;padding:12px 16px;border-left:3px solid #6366f1;background:#f8f8ff;color:#444;font-style:italic;border-radius:4px;">${payload.textSnippet}…</blockquote>`
    : "";
        ...
        <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#6366f1,#8b5cf6);padding:32px 40px;">
              <h1 style="margin:0;color:#fff;font-size:22px;font-weight:700;">✅ Transcripción lista</h1>
              <p style="margin:6px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">
                Tu transcripción ${typeLabel} ha finalizado correctamente.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:32px 40px;">
              <p style="margin:0 0 8px;color:#111;font-size:15px;">Hola, <strong>${payload.userName}</strong>.</p>
              <p style="margin:0 0 20px;color:#555;font-size:14px;line-height:1.6;">
                Tu transcripción en <strong>${payload.language.toUpperCase()}</strong> ya está disponible.
              </p>

              ${snippet}

              <a href="${payload.detailUrl}"
          ...
          <tr>
            <td style="padding:20px 40px;border-top:1px solid #f0f0f0;">
              <p style="margin:0;color:#aaa;font-size:12px;">
                JustWriteIt · Este email se envió automáticamente, no respondas a este mensaje.
              </p>
            </td>
          </tr>
...
export async function notifyTranscriptionCompleted(payload: NotifyPayload): Promise<{
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL; // ej: "JustWriteIt <noreply@justwriteit.app>"
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      subject: "✅ Tu transcripción está lista — JustWriteIt",
